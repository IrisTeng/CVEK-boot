...
verify <- 
  function(temp){
    ...
    dd <- KernelBoot(formula, label.names, Kernlist, rawData)
    
    pvalue <- dd[[1]]
    bs.test <- dd[[2]]
    original.test <- dd[[3]]
    return(c(pvalue, bs.test, original.test))
  }
    ...
    for (method in c("linear", "rbf", "polynomial")) {
      for (int.effect in c(0, 0.1, 0.2, 0.3)) {
          for (B in c(100)) {
            ...
            result <- rbind(result, c(int.effect, method, n, res2))
            cat(c(int.effect, method, n, res2),
