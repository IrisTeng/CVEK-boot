...
# 5th library: 3 polynomials with different degree
Kernlist <- NULL
Kernlist <- c(Kernlist, kernelGenerate('matern', p = 0))
Kernlist <- c(Kernlist, kernelGenerate('matern', p = 1))
Kernlist <- c(Kernlist, kernelGenerate('matern', p = 2))
                ...
                if(method == "matern"){
                  ...
                  for(l in c(0.5, 1, 1.5)){
                    ...
                    cat(c(method, p, l, int_effect, mode, strategy, n, res2),
                        file = "simulation_power_a5.txt", append = T, "\n")
...
write.csv(result, file = "simulation_power_a5.csv", row.names = F, quote = F)
