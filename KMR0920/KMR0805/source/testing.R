...
testing <- function(formula_int, label_names, Y, X1, X2, Kernlist,
  ...
  if (test == "boot"){
    ...
    tau_ret <- original_ans$tau
    dif_det <- determinant(original_ans$dif %*% t(original_ans$dif))
    V0_inv_det <- determinant(original_ans$V0_inv)
    K12_det <- determinant(original_ans$K12)
    
    # dif_vec <- original_ans$dif
    ...
    out <- list(pvalue = pvalue, u_weight = u_weight,
                ...
                tau_ret = tau_ret, dif_det = dif_det, 
                V0_inv_det = V0_inv_det, K12_det = K12_det)
  }
  else if (test == "asym"){
    asym_ans <-
      scoreStat(n, Y, X12, beta0, sigma2_hat, tau_hat, K_gpr)
    
    tau_ret <- asym_ans$tau
    dif_det <- determinant(asym_ans$dif %*% t(asym_ans$dif))
    V0_inv_det <- determinant(asym_ans$V0_inv)
    K12_det <- determinant(asym_ans$K12)
