...
testing <- function(formula_int, label_names, Y, X1, X2, kern_list,
                    ...
                    test = "boot", lambda_list = exp(seq(-10, 5, .5)), 
                    B = 100) {
...
test_boot <- function(n, Y, X12, beta0, alpha0,
  ...
  bs_test <- sapply(1:B, function(k) {
    ...
    sigma2_star <- estimate_noise(Ystar, lam_star, beta0, alpha0, K_gpr)
    tau_star <- sigma2_star / lam_star
    compute_stat(n, Ystar, X12, beta0, K_gpr, sigma2_star, tau_star)
  })
