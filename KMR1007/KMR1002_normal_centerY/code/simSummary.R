setwd('/home/wd45/KMR0928/output/Temp')
      ...
      for (mode in c("AIC", "BIC", "GCV", "AICc", "GCVc", "gmpml", "loocv")) {
        for (strategy in c("avg")) {
          for (int_effect in c(seq(0, .29, .05), seq(.3, 1, .1))) {
            ...
            sim_power <- mean(pvalue_temp < .05, na.rm = TRUE)
            sim2_power <- mean(pvalue2_temp < .05, na.rm = TRUE)
            lam_mean <- round(median(lam_temp), 4)
            K_mean <- round(mean(K_temp), 4)
            A_mean <- round(mean(A_temp), 4)
            V0_invlog_mean <- round(mean(V0_invlog_temp, na.rm = TRUE), 4)
